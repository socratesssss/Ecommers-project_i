...
const ProductPage = () => {
  ...
  try {
    ...
    if (!res.ok) {
      // If response is not OK (e.g. 404 or 500), throw error with message
      const text = await res.text(); // read as plain text
      throw new Error(`Fetch error: ${res.status} - ${text}`);
    }

    const data = await res.json(); // parse JSON only if response.ok
    setProduct(data);
  } catch (err) {
 ...
 const getDisplayImages = () => {
  ...
  // 🧠 You can keep the full UI JSX below (same as before) using `product`
